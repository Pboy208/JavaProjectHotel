...
public class LoginController implements Initializable {
	...
	public void signInButtonOnAction(ActionEvent event) throws Exception {
		...
		if (clientType.isSelected()) {
			user_id = CheckPasswordDB.checkPasswordClients(accountNameString, passwordString);
			if (user_id == -1) {
			...
			user_id = CheckPasswordDB.checkPasswordHotels(accountNameString, passwordString);
			if (user_id == -1) {
				alertLabel.setText("Wrong user name or password");
				return;
			}
