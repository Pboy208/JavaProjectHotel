...
public class HotelInfoView implements Initializable {
	...
	public void saveChange(ActionEvent event) throws SQLException {
		...
		// -------------------------------------- User info
		HotelEmployees user = (HotelEmployees) LoginView.getUser();
		...
		for (int i = 0; i < 12; i++)
			if (rbs[i].isSelected())
				extensions[i] = 1;
		// -------------------------------------- Update hotel info
		HotelDB.updateHotelInfo(user.getHotelID(), hotelNameString, hotelAddressString, starInt, extensions,priceFloat);
	}
	...
	public void cancelReceipt(ActionEvent event) throws SQLException {
		...
		ReceiptDB.updateReceiptStatusHotels(((HotelEmployees)LoginView.getUser()).getHotelID());
		ArrayList<Receipts> receipts = ReceiptDB.queryReceiptsForHotel(((HotelEmployees)LoginView.getUser()).getHotelID());
		if (receipts == null) {
