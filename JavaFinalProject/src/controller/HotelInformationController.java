...
public class HotelInformationController implements Initializable {
	...
	public void back(ActionEvent event) {
		new SceneChanging().changeScene(event, "HostInformation.fxml");
	}
	...
	public void saveChange(ActionEvent event) throws SQLException {
		...
		hotel.setHotelID(HostInformationController.getHotel().getHotelID());
	...
	public void cancelReceipt(ActionEvent event) throws SQLException {
		...
		if (chosenReceipt.getStatus() == 1) {
			alertViewDetails.setText("Can not cancel receipt that are being taken");
			return;
		}
		if (chosenReceipt.getStatus() == -1) {
	...
	public void addRoom(ActionEvent event) throws SQLException {
		...
		continueConfirmation.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				try {
					Rooms room = new Rooms(HostInformationController.getHotel().getHotelID());
					room.addRoom();
	...
	public void initialize(URL arg0, ResourceBundle arg1) {
		...
		Hotels hotel = new Hotels();
		try {
			hotel.queryInstance(HostInformationController.getHotel().getHotelID());
		} catch (SQLException e) {
	...
	private void refreshRoom() {
		...
		try {
			allRooms = Rooms.queryAllRooms(HostInformationController.getHotel().getHotelID());
		} catch (SQLException e) {
			e.printStackTrace();
		}
