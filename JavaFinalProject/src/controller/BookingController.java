...
public class BookingController implements Initializable {
	...
	public void clientInfo(ActionEvent event) {
		new SceneChanging().changeScene(event, "UserInformation.fxml");
	}
	...
	public void checkForTotalPrice(ActionEvent event) throws SQLException {
		...
		int total = Rooms.calculatePayment(numberOfRoomInt, hotel.getHotelID(), checkinDate, checkoutDate,hotel.getPrice());
	...
	public void confirmBooking(ActionEvent event) throws SQLException {
		Hotels hotel = recommendHotels.getSelectionModel().getSelectedItem();
		try {
			Integer.parseInt(numberOfRoom.getText());
		} catch (Exception e) {
			alert.setText("Number of room must be a number");
			return;
		}
		...
		continueConfirmation.setOnAction(new EventHandler<ActionEvent>() {
			...
			public void handle(ActionEvent arg0) {
				try {
					Receipts.addReceipt(Integer.parseInt(numberOfRoom.getText()), hotel.getHotelID(), checkinDate, checkoutDate,LoginController.getUser().getUserID());
	...
	private void openConfirmationPane() {
		bookingPane.setEffect(new GaussianBlur(20));
