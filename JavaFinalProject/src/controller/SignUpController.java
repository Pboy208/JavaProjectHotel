...
public class SignUpController implements Initializable {
	...
	private void clearTextFields() {
		...
		districtBox.setPromptText("Choose a district");
		streetBox.setPromptText("Choose a street");
	...
	public void signUpUser(ActionEvent event) throws SQLException {
		...
		newUser.setName(nameC.getText());
		newUser.setPhone(phoneC.getText());
		newUser.setEmail(emailC.getText());
		newUser.setPassword(pwC.getText());
		newUser.setUsername(accountNameC.getText());
		String passwordConfirm = pwConfirmationC.getText();
		

		if (!Functions.checkPhoneNumber(newUser.getPhone())) {
			alertLabel.setVisible(true);
		...
		if (!Functions.checkEmail(newUser.getEmail())) {
			alertLabel.setVisible(true);
		...
		int result = newUser.signUpUserProcedure();
	...
	public void signUpHotelManager(ActionEvent event) throws SQLException {
		...
		newManager.setName(nameH.getText());
		newManager.setPhone(phoneH.getText());
		newManager.setEmail(emailH.getText());
		newManager.setPassword(pwH.getText());
		newManager.setUsername(accountNameH.getText());
		...
		newHotel.setName(hotelName.getText());;
		newHotel.setAddress(specificAddressString + "," + street + "," + district + "," + province);
		newHotel.setStreetID(streetID);
		
		if (!Functions.checkPhoneNumber(newManager.getPhone())) {
			alertLabel.setVisible(true);
			alertLabel.setText("Phone number must be a sequence of 10 numbers");
			phoneH.clear();
			return;
		...
		if (!Functions.checkEmail(newManager.getEmail())) {
			alertLabel.setVisible(true);
			alertLabel.setText("Invalid email address");
			emailH.clear();
			return;
		...
		if (!newManager.getPassword().equals(passwordConfirm)) {
			alertLabel.setText("Password confirmation is not match");
			alertLabel.setVisible(true);
			pwConfirmationH.clear();
			pwH.clear();
		...
		int result = newManager.signUpHotelManagerProcedure(newHotel);
